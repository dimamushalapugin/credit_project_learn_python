...
def equipment_valute(pl_new_or_not: str, investor: str, equipment_or_not: str, pnr: str, currency: str, stock: str):
    ...
    return (new_old_pl, pb_vizor, identif_punkt_3_1_1, identif_punkt_3_1_3, punkt_3_1_9, punkt_3_3_3_key,
            punkt_3_3_3_key2, punkt_3_1_6, punkt_3_3_7, punkt_8_2, pril_1_2, pril_1_3, punkt_4_7_1, punkt_5_3, punkt_6_4,
            punkt_8_3, kurs, type_currency, equivalent_currency)

def quantity_payment_order(payment_order: str):
    sum = 0
    ...
    if sum == 100:
        ...
        print("Суммы не равны 100, надо написать пользователю что он даун и не умеет считать!!! Flash должен выскочить")
    return sum
...
def number_to_words(suma_chislo123, currency: str):
    try:
        # Разбиваем строку на целую и десятичную часть
...
def payment_for_dkp(payment_order: str, price_entry):
    print(payment_order)
    punkt_2_3_1_pay = ''
    punkt_2_3_2_pay = ''
    punkt_2_3_3_pay = ''
...
def create_value_seller():
    ...
    return (leader_seller, imenyemoe_dkp, leader_seller_rod_padezh, put_padezh_podpisant_seller, deystvuysh_list_seller,
    doverka_ustav_seller, leader_leasee_pod, leader_seller_rod_padezh, formatted_name_leader_seller)

def result_dadata(inn_seller: str):
    result_dkp = dadata.find_by_id("party", inn_seller)
    return result_dkp
...
def full_rekviti_seller(result_dkp, inn_seller):
    ip_or_kfh_dkp = 'Нет'
...
def seller_dkp_all(inn_seller: str):
    result_dkp = dadata.find_by_id("party", inn_seller)
...
def deistv_seller(result_dkp):
    ...
    return deystvuysh_list_seller, deystvuysh_list_seller, imenyemoe_dkp

    # Чтение заявки и анкеты
def read_xlsx_dkp(inn_leasee, path_application):
    ip_or_kfh = 'Нет'
    ...
    for number in range(24, sheet_zayavlenie.max_row + 2):
        if sheet_zayavlenie[
            f'B{number}'].value == ('Место эксплуатации предмета лизинга (для автотранспорта место стоянки/хранения) '
    ...
    # print(
    #     f"Здесь брать предмет лизинга {predmet_lizinga}, ИНН поставщика {inn_seller_list}
    #     и цену {price_predmet_lizinga}")
    ...
    for i in range(3, sheet_zayavlenie.max_row + 2):
        if sheet_zayavlenie[f'B{i}'].value == 'Наименование поставщика':
            ...
            while sheet_zayavlenie[
                f'A{number}'].value != ('и передать нашей организации/ индивидуальному предпринимателю в финансовую '
    ...
    # print(
    #     f"Здесь брать название поставщика {seller_title}, ИНН поставщика {inn_seller_list2} и адрес поставщика
    #     {seller_address}")
    ...
    logging.info(f'({current_user}) Все данные успешно прочитаны') # эту штуку надо оставить?
    application_name = fr'{dir_path}\Заявка с заключением {inn_leasee}.xlsx' # эту штуку надо оставить?
    ...
    date_regist, okpo_leasee, okato_leasee, ogrn_leasee, inn_seller_list, price_predmet_lizinga, predmet_lizinga,
...
def replace():
    ...
    new_words_dkp = [str(new_old_pl), str(pb_vizor), str(identif_punkt_3_1_1), str(identif_punkt_3_1_3),
                     ...
                     str(punkt_2_3_4_numb_pr), str(punkt_2_3_5_numb_pr), str(payment_1), str(payment_2), str(payment_3),
                     ...
                     str(equivalent_currency), str(full_name_seller), str(imenyemoe_dkp), str(leader_seller_rod_padezh),
                     ...
                     ogrn_leasee, inn_seller_list, price_predmet_lizinga, predmet_lizinga, formatted_name_leader_leasee,
