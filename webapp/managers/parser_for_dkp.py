...
def start_filling_agreement_dkp(path_application: str, inn_client: str, inn_seller: str, numb_dl_dkp: str,
    ...
    def replace_words_in_dkp(docx_file, old_words_dkp, new_words_dkp):
        ...
        if len(payment_order.split(' ')) == 1:
            for run in doc.paragraphs:
                if run.text.strip() in [
                    f'2.3.1. {payment_dkp[0]} платеж в размере {payment_dkp[5]}% ({payment_dkp[10]}) от стоимости предмета лизинга, что составляет сумму {eq_val[-2]} {payment_dkp[15]:,.2f}'.replace(".", ",") + ' ' + f'{payment_dkp[20]}, в т.ч. НДС 20%, в течение 15 (пятнадцати) рабочих дней с момента перечисления Лизингополучателем первоначального платежа Покупателю по договору лизинга, а также после выставления Продавцом счета на оплату.',
                    f'2.3.2. {payment_dkp[1]} платеж в размере {payment_dkp[6]}% ({payment_dkp[11]}) от стоимости предмета лизинга, что составляет сумму {eq_val[-2]} {payment_dkp[16]:,.2f}.replace(".", ",") {payment_dkp[21]}, в т.ч. НДС 20%, в течение 7 (семи) рабочих дней с момента получения уведомления о готовности предмета лизинга к отгрузке согласно п.3.1.1 и после выставления Продавцом счета на оплату.',
                    f'2.3.3. {payment_dkp[2]} платеж в размере {payment_dkp[7]}% ({payment_dkp[12]}) от стоимости предмета лизинга, что составляет сумму {eq_val[-2]} {payment_dkp[17]:,.2f}.replace(".", ",") {payment_dkp[22]}, в т.ч. НДС 20%, в течение 7 (семи) рабочих дней после выставления Продавцом счета на оплату.',
                    f'2.3.4. {payment_dkp[3]} платеж в размере {payment_dkp[8]}% ({payment_dkp[13]}) от стоимости предмета лизинга, что составляет сумму {eq_val[-2]} {payment_dkp[18]:,.2f}.replace(".", ",") {payment_dkp[23]}, в т.ч. НДС 20%, в течение 7 (семи) рабочих дней после выставления Продавцом счета на оплату.',
                    f'2.3.5. {payment_dkp[4]} платеж в размере {payment_dkp[9]}% ({payment_dkp[14]}) от стоимости предмета лизинга, что составляет сумму {eq_val[-2]} {payment_dkp[19]:,.2f}.replace(".", ",") {payment_dkp[24]}, в т.ч. НДС 20%, в течение 7 (семи) рабочих дней после выставления Продавцом счета на оплату.']:
                    doc.element.body.remove(run._element)

        elif len(payment_order.split(' ')) == 2:
            for run in doc.paragraphs:
                if run.text.strip() in [
                    f'2.3.1. Оплата в размере {payment_dkp[5]}% ({payment_dkp[10]}) от стоимости предмета лизинга, что составляет сумму {eq_val[-2]} {payment_dkp[15]:,.2f}.replace(".", ",") {payment_dkp[20]}, в т.ч. НДС, в течение 15 (пятнадцати) рабочих дней с момента перечисления Лизингополучателем первоначального платежа Покупателю по договору лизинга,',
            ...
            for run in doc.paragraphs:
                if run.text.strip() in [
                    f'2.3.1. Оплата в размере {payment_dkp[5]}% ({payment_dkp[10]}) от стоимости предмета лизинга, что составляет сумму {eq_val[-2]} {payment_dkp[15]:,.2f}'.replace(".", ",") + ' ' + f'{payment_dkp[20]}, в т.ч. НДС, в течение 15 (пятнадцати) рабочих дней с момента перечисления Лизингополучателем первоначального платежа Покупателю по договору лизинга,',
                    'и в течение 7 (семи) рабочих дней с момента получения Покупателем уведомления Продавца о готовности предмета лизинга к отгрузке согласно п.3.1.1 и после выставления Продавцом счета на оплату.']:
                    doc.element.body.remove(run._element)
