...
def full_name_company(soup):
    try:
        return soup.find("p", class_="cards__text cards__text-margin").get_text(
            strip=True
        )
...
def short_name_company(soup):
    try:
        return (
            soup.find("h2", class_="cards__company").find("span").get_text(strip=True)
        )
    except Exception as _ex:
        logging.info(_ex, exc_info=True)
        return "-"
...
def company_status(soup):
    try:
        if soup.find("span", class_="cards__status cards__status-green") is not None:
            return (
                soup.find("span", class_="cards__status cards__status-green")
                .get_text(strip=True)
                .replace("По данным ФНС:", "")
            )
        elif soup.find("span", class_="cards__status cards__status-red") is not None:
            return (
                soup.find("span", class_="cards__status cards__status-red")
                .get_text(strip=True)
                .replace("По данным ФНС:", "")
            )
...
def legal_address(soup):
    try:
        return soup.find(
            "span", class_="find_address cards__column_block-link"
        ).get_text(strip=True)
    except Exception as _ex:
        logging.info(_ex, exc_info=True)
        return "-"
...
def pfr(soup):
    try:
        return (
            soup.find("tr", title="Пенсионный фонд России")
            .find_all("td")[1]
            .get_text(strip=True)
        )
    except Exception as _ex:
...
def fss(soup):
    try:
        return (
            soup.find("tr", title="Social Insurance Fund")
            .find_all("td")[1]
            .get_text(strip=True)
        )
    except Exception as _ex:
...
def director_name(soup):
    try:
        return (
            soup.find("div", "cards__text cards__text-top")
            .find("a", class_="cards__column_block-link")
            .get_text(strip=True)
        )
    except Exception as _ex:
        logging.info(_ex, exc_info=True)
...
def director_href(soup):
    try:
        return soup.find("div", "cards__text cards__text-top").find(
            "a", class_="cards__column_block-link"
        )["href"]
    except Exception as _ex:
        logging.info(_ex, exc_info=True)
        return "-"
...
def director_inn_egrul(soup) -> List[str]:
    try:
        elements = soup.find("div", "cards__text cards__text-top").find_all(
            "span", class_="cards__column-small"
        )
...
def authorized_capital(soup):
    try:
        elements = soup.find(
            "div", class_="cards__column cards__column-first"
        ).find_all("div", class_="cards__column_block")
        text = ""
        pattern = r"(\d[\d\s]+руб\.)"

        for element in elements:
            if "Уставный капитал" in element.get_text(strip=True):
                text = element.get_text(strip=True, separator=" ").removeprefix(
                    "Уставный капитал: "
                )
...
def main_activity(soup):
    try:
        code = (
            soup.find(class_="cards__column_block-link cards__column_more-link appear")
...
def additional_activities(soup) -> List[str]:
    try:
        elements = soup.find(
            "table",
            class_="cards__data-small cards__data-nomargin cards__column_hidden",
        ).find_all("tr")
...
def founders_physical(soup) -> dict:
    ...
    try:
        for num, element in enumerate(elements, start=1):
            founders[num] = {}
            percent = (
                element.find("td").get_text(strip=True) if element.find("td") else ""
            )
            try:
                egrul = (
                    element.find("div", class_="division")
                    .find("span", class_="cards__column-small")
                    .get_text(strip=True)
            ...
            try:
                href = (
                    element.find("div", class_="division").find(
                        "a", class_="cards__column_block-link"
                    )["href"]
                    if element.find("div", class_="division").find(
                        "a", class_="cards__column_block-link"
            ...
            try:
                sum_ = (
                    re.sub(
                        ...
                        " ",
                        element.find("div", class_="division")
                        .find_all("span", class_="cards__column-small")[1]
                        .get_text(strip=True),
                    )
                    if element.find("div", class_="division").find_all(
                        "span", class_="cards__column-small"
                    )[1]
                    else ""
                )
            ...
            try:
                inn = (
                    element.find("div", class_="division")
                    .find_all("span", class_="cards__column-small")[2]
                    .get_text(strip=True)
                    if element.find("div", class_="division").find_all(
                        "span", class_="cards__column-small"
                    )[2]
            ...
            mass_founder = (
                element.find("div", class_="division")
                .find("div", class_="link-red")
                .get_text(strip=True)
                if element.find("div", class_="division").find("div", class_="link-red")
                else ""
            )
...
def founders_legal(soup) -> dict:
    ...
    try:
        for num, element in enumerate(elements, start=1):
            ...
            try:
                sum_ = (
                    re.sub(
                        ...
                        " ",
                        element.find("div", class_="division")
                        .find_all("span", class_="cards__column-small")[0]
                        .get_text(strip=True),
                    )
                    if element.find("div", class_="division").find_all(
                        "span", class_="cards__column-small"
            ...
            name = (
                element.find("div", class_="division")
                .find("a", class_="cards__column_block-link open-card")
                .get_text(strip=True)
                if element.find("div", class_="division").find(
                    "a", class_="cards__column_block-link open-card"
                )
            ...
            if element.find("div", class_="division").find(
                "span",
                class_="cards__status cards__status_description cards__status-green",
            ):
                status = (
                    element.find("div", class_="division")
                    .find(
            ...
            elif element.find("div", class_="division").find(
                "span",
                class_="cards__status cards__status_description cards__status-orange",
            ):
                status = (
                    element.find("div", class_="division")
                    .find(
                ...
                "span",
                class_="cards__status cards__status_description cards__status-red",
            ):
                status = (
                    element.find("div", class_="division")
                    .find(
            ...
            try:
                ogrn = (
                    element.find("div", class_="division")
                    .find_all("span", class_="cards__column-small")[3]
                    .get_text(strip=True)
                    if element.find("div", class_="division").find_all(
                        "span", class_="cards__column-small"
                    )[3]
...
def blocked_current_accounts(soup):
    try:
        return " ".join(
            soup.find("div", class_="card card-nopadding external-111")
            .find("div", class_="card-section")
            .find("span")
            .get_text(strip=True)
            .split()
        )
...
def tax_debts(soup) -> List[str]:
    ...
    try:
        link_green = (
            soup.find(
                "div",
                class_="card card-nopadding",
                attrs={"data-element": "local-reestr"},
            )
            .find("p", class_="link-green")
    ...
    try:
        ...
        for elem in link_red:
            if "По данным ФНС на дату" not in " ".join(
                elem.get_text(strip=True, separator=" ").split()
            ):
    ...
    try:
        tax_arrears = " ".join(
            soup.find(
                "div",
                class_="card card-nopadding",
                attrs={"data-element": "local-tax-penalty"},
            )
            .find("div", class_="card-section")
            .find("p")
            .get_text(strip=True, separator=" ")
...
def history(soup):
    try:
        elements = (
            soup.find("div", class_="card-wrapper")
            .get_text(strip=True, separator="|")
            .replace("\n", " ")
            .replace("  ", "")
            .replace("|", "\n")
                ...
                try:
                    ...
                    if element.find("i", class_="link-up"):
                        ...
                        fin[year].setdefault(
                            ...
                            .strip()
                            .removesuffix(
                                element.find("i", class_="link-down").get_text(
                                    strip=True
                                )
                            ),
                        )
                        ...
                        fin[year].setdefault(
                            find_element[0].strip(), find_element[1].strip()
                        )
                except (AttributeError, TypeError, IndexError) as _ex:
    ...
    try:
        ...
        elements = (
            soup.find(
                "div",
                class_="card card-nopadding",
                attrs={"data-element": "local-taxees-fees"},
            )
            .find("div", class_="card-section")
            .find_all("p")
        )
...
def federal_resource(soup):
    try:
        return (
            soup.find("div", class_="card card-nopadding external-217")
            .find("div", class_="card-section")
            .get_text(strip=True, separator=" ")
            .removesuffix("Подробнее")
            .strip()
        )
    except Exception as _ex:
        logging.info(_ex, exc_info=True)
...
def register_of_pledges(soup):
    try:
        return (
            soup.find("div", class_="card card-nopadding external-157")
            .find("div", class_="card-section")
            .get_text(strip=True, separator=" ")
            .removesuffix("Подробнее")
            .removesuffix("Предыдущие данные")
            .strip()
        )
    except Exception as _ex:
        logging.info(_ex, exc_info=True)
...
def rosfinmonitoring(soup):
    try:
        return (
            soup.find("div", class_="card card-nopadding external-142")
            .find("div", class_="card-section")
            .get_text(strip=True, separator=" ")
            .replace("Подробнее", "")
            .strip()
    ...
    try:
        return (
            soup.find(
                "div",
                class_="card card-nopadding",
                attrs={"data-element": "local-sanction"},
            )
            .find("div", class_="card-section")
            .get_text(strip=True, separator=" ")
        )
...
def get_url_delta(soup):
    try:
        return soup.find(
            "div",
            class_="card card-nopadding",
            attrs={"data-element": "local-analitic"},
        ).find("a", class_="cards__column_block-link")["href"]
    except Exception as _ex:
        logging.info(_ex, exc_info=True)
        return "-"
...
def inaccuracy_of_information(soup):
    try:
        if (
            "недостоверн"
            in soup.find("div", class_="cards__column cards__column-first")
            .get_text(" ", strip=True)
            .lower()
        ):
            return "Да"
...
def mass_address(soup):
    try:
        if soup.find("div", class_="cards__column_block address_info").find(
            "p", class_="link-red"
        ):
            return "Да"
        else:
...
def extremism(soup):
    ...
    try:
        if (
            "источнику отсутствует" in status
            or "Запрос по источнику не отправлялся" in status
        ):
            return "Нет"
        else:
            return "Да"
    ...
    try:
        return " ".join(
            soup.find(
                "div",
                class_="card card-nopadding",
                attrs={"data-element": "local-founded-companies"},
            )
            .find("div", class_="card-section")
            .get_text(" ", strip=True)
            .split()
...
def read_main_html(client_inn, object_inn, short_name):
    try:
        soup = BeautifulSoup(
            read_main_page(client_inn, object_inn, short_name), "html.parser"
        )
    except Exception as _ex:
        logging.info(_ex, exc_info=True)
        raise _ex
    ...
    general_description_of_the_company = {
        ...
        "Убыточность (да_нет)": unprofitability(
            financial_statements(soup)
        ),  # for seller table
        "Недостоверность сведений (да_нет)": inaccuracy_of_information(
            soup
        ),  # for seller table
...
def read_main_html_individual(client_inn, object_inn, short_name, individual=False):
    if not individual:
        ...
        try:
            soup = BeautifulSoup(
                read_main_page(client_inn, object_inn, short_name, True), "html.parser"
            )
        except FileNotFoundError as _ex:
            logging.info(_ex, exc_info=True)
            raise _ex
    ...
    def ind_main_profile(title):
        try:
            status = (
                soup.find("table", class_="cards__data contact_info")
                .find("td", string=title)
                .find_next("td")
                .get_text(" ", strip=True)
            )
            logging.info(status)
            ...
            logging.info(_ex, exc_info=True)
            logging.info(
                f"Не прожата кнопка для получения инфы по паспорту и тд. {title}"
            )
            return "-"
    ...
    def ind_fio():
        try:
            status = " ".join(
                soup.find("h2", class_="cards__company")
                .find("span")
                .get_text(" ", strip=True)
                .split()
            )
    ...
    def ind_entrepreneur():
        ...
        if len_ornip > 0:
            for num in range(1, len_ornip + 1):
                ...
                try:
                    date_of_reg = (
                        soup.find("div", class_="cards__column cards__column-first")
                        .find_all("table", class_="cards__data")[num]
                        .find_all("td")[1]
                        .get_text(" ", strip=True)
                    )
                ...
                try:
                    date_of_liquidation = (
                        soup.find("div", class_="cards__column cards__column-first")
                        .find_all("table", class_="cards__data")[num]
                        .find("td", string="Дата прекращения деятельности:")
                        .find_next("td")
                        .get_text(" ", strip=True)
                    )
    ...
    def ind_address():
        try:
            address = (
                soup.find("h3", class_="cards__subtitle", string="Адрес регистрации:")
                .find_next("div", class_="cards__column_block")
                .get_text(" ", strip=True)
                .replace("посмотреть на карте", "")
            )
            return address
        except Exception as _ex:
            logging.info(_ex, exc_info=True)
    ...
    def ind_tax_authority():
        try:
            ...
            logging.info(_ex, exc_info=True)
            logging.info(
                "Не удалось получить информацию по наименованию налогового органа"
            )
            return "-"
    ...
    def ind_is_dir_or_founder_history():
        info = {}
        try:
            is_director = soup.find("div", class_="cards-directors").find_all(
                "div", class_="company_more_info"
            )
            info["Является руководителем"] = "\n".join(
                [el.get_text(" ", strip=True) for el in is_director]
            )
        except Exception as _ex:
            logging.info(_ex, exc_info=True)
            info["Является руководителем"] = "-"
        ...
        try:
            is_founder = soup.find(
                "table",
                class_="cards__data cards__data-small cards__data-border founder-table cards-founders",
            ).find_all("tr")
            info["Является учредителем"] = "\n".join(
                [el.get_text(" ", strip=True) for el in is_founder]
            )
        except Exception as _ex:
            logging.info(_ex, exc_info=True)
            info["Является учредителем"] = "-"
        ...
        try:
            was_director = soup.find("div", class_="directors-history").find_all(
                "div", class_="company_more_info"
            )
            info["Являлся руководителем"] = "\n".join(
                [el.get_text(" ", strip=True) for el in was_director]
            )
        except Exception as _ex:
            logging.info(_ex, exc_info=True)
            info["Являлся руководителем"] = "-"
        ...
        try:
            was_founder = soup.find(
                "table",
                class_="founders-history cards__data cards__data-small cards__data-border founder-table",
            ).find_all("tr")
            info["Являлся учредителем"] = "\n".join(
                [el.get_text(" ", strip=True) for el in was_founder]
            )
        except Exception as _ex:
            logging.info(_ex, exc_info=True)
            info["Являлся учредителем"] = "-"
        ...
        try:
            arbitrary = " ".join(
                soup.find(
                    "div",
                    class_="card card-nopadding",
                    attrs={"data-element": "local-arbitr"},
                )
                .find("div", class_="card-section")
                .get_text(" ", strip=True)
                .split()
    ...
    def ind_reestr_zalog():
        try:
            reestr = " ".join(
                soup.find("div", class_="card card-nopadding external-158")
                .find("div", class_="card-section")
                .find("span")
                .get_text(" ", strip=True)
                .split()
    ...
    def ind_fed_res():
        try:
            fed = " ".join(
                soup.find("div", class_="card card-nopadding external-218")
                .find("div", class_="card-section")
                .find("span")
                .get_text(" ", strip=True)
                .split()
    ...
    def ind_narcos():
        try:
            narcos = " ".join(
                soup.find("div", class_="card card-nopadding external-117")
                .find("div", class_="card-section")
                .find("span")
                .get_text(" ", strip=True)
                .split()
    ...
    def ind_ros_fin():
        try:
            ros = " ".join(
                soup.find("div", class_="card card-nopadding external-143")
                .find("div", class_="card-section")
                .find("span")
                .get_text(" ", strip=True)
                .split()
    ...
    def ind_justice():
        try:
            just = " ".join(
                soup.find("div", class_="card card-nopadding external-164")
                .find("div", class_="card-section")
                .find("span")
                .get_text(" ", strip=True)
                .split()
    ...
    def ind_passport():
        try:
            passport = " ".join(
                soup.find("div", class_="card card-nopadding external-175")
                .find("div", class_="card-section")
                .find("span")
                .get_text(" ", strip=True)
                .split()
    ...
    def ind_debtor():
        try:
            debtor = " ".join(
                soup.find("div", class_="card card-nopadding external-50")
                .find("div", class_="card-section")
                .find("span")
                .get_text(" ", strip=True)
                .split()
    ...
    def ind_period_of_activity():
        try:
            date_of_reg = (
                soup.find("div", class_="cards__column cards__column-first")
                .find_all("table", class_="cards__data")[1]
                .find_all("td")[1]
                .get_text(" ", strip=True)
            )
        except Exception as _ex:
    ...
    def ind_bankrupt_notices():
        try:
            if soup.find(
                "div",
                class_="card card-nopadding",
                attrs={"data-element": "local-bankrupt"},
            ).find("div", class_="card-section").get_text(
    ...
    def ind_fssp():
        try:
            ...
            if (
                "Информация по данному источнику отсутствует" == fssp
                or "Информация по источнику отсутствует" == fssp
            ):
                return "Нет"
        ...
        try:
            gov = " ".join(
                soup.find(
                    "div",
                    class_="card card-nopadding",
                    attrs={"data-element": "local-state-contracts"},
                )
                .find("div", class_="card-section")
                .get_text(" ", strip=True)
                .split()
