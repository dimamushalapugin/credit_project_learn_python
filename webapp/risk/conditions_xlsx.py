        ...
        for cell in row:
            if cell.value == "Дата регистрации":
                ...
                try:
                    ...
                    cur_date_seller = dt.strptime(
                        dt.now().strftime("%d.%m.%Y"), "%d.%m.%Y"
                    )
    ...
    for row in sheet.iter_rows():
        for cell in row:
            if cell.value == "Уставный капитал":
                ...
                try:
                    capital = float(
                        adjacent_cell.value.replace(" руб.", "").replace(" ", "")
                    )
                    if capital >= 100_000:
                        adjacent_cell.font = font_green
                    else:
...
def delta_rating(sheet):
    color_green = PatternFill(
        start_color="a7cfa7", end_color="a7cfa7", fill_type="solid"
    )
    color_orange = PatternFill(
        start_color="fabb7c", end_color="fabb7c", fill_type="solid"
    )
