...
from webapp.risk.new_create_risk_conclusion import create_conclusion, create_conclusion_individual
...
def get_folder_names_individual(folder_path):
    ...
    absolute_folder_path = os.path.join('webapp', folder_path)
    if os.path.exists(absolute_folder_path) and os.path.isdir(absolute_folder_path):
        individual_folder_names = [item for item in os.listdir(absolute_folder_path) if
                                   os.path.isdir(os.path.join(absolute_folder_path, item))]
        individual_folder_names.sort(key=lambda x: os.path.getmtime(os.path.join(absolute_folder_path, x)),
                                     reverse=True)
...
def create_risk_conclusion_individual():
    ...
    try:
        ...
        if file_name_individual:
            flash(f'Файл успешно создан', 'success')
            return redirect(url_for('risk.risk_page', file_name_individual=file_name_individual))
        else:
            return redirect(url_for('risk.risk_page', file_name_individual=file_name_individual))
    except Exception as e:
        flash(str(e), 'error')
