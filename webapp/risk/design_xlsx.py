...
def main_design(file_name):
    ...
    alignment_center_bold = Alignment(horizontal='center', vertical='center', wrap_text=True)
    alignment_right = Alignment(horizontal='right', vertical='center', wrap_text=True)
    alignment_top_left_bold = Alignment(horizontal='left', vertical='top', wrap_text=True)
    ...
    for row in sheet.iter_rows():
        for cell in row:
            ...
            if cell.value in ['Наименование поля', 'Значение', 'Рейтинг', 'Описание']:
                ...
                cell.font = font_bold
            if cell.value in ['Статус компании:', 'Дата регистрации:',
                              'Документы, поданные для регистрации/изменения в ЕГРЮЛ:',
                              'Адрес регистрации организации:', 'Размер уставного капитала:',
                              'Массовость директоров/учредителей:', 'Дисквалифицированные лица:',
                              'Преступления, связанные с оборотом наркотиков по данным УФСКН России:',
                              'Смена директоров/учредителей:', 'Госконтракты:', 'Сообщения о банкротстве:',
                              'Налоговая задолженность и отчетность:', 'Заблокированные расчетные счета:',
                              'Арбитражные дела:', 'Исполнительные производства:', 'Наличие дочерних компаний:',
                              'Реестр недобросовестнных поставщиков:']:
                cell.font = font_bold
                cell.alignment = alignment_wrap
            if cell.value in ['Анализ регистрационных данных', 'Анализ директоров/учредителей', 'Анализ деятельности']:
                cell.font = font_bold
                cell.fill = color_delta
                adjacent_cell = sheet.cell(row=cell.row, column=cell.column + 1)
                adjacent_cell.fill = color_delta
            if cell.value in ['ДОЧЕРНИЕ ОРГАНИЗАЦИИ', 'НАЛОГИ И СБОРЫ',
                              'АФФИЛИРОВАННЫЕ И СВЯЗАННЫЕ ЛИЦА', 'ПРОВЕРКА АФФИЛИРОВАННЫХ КОМПАНИЙ',
                              'СУЩЕСТВЕННЫЕ ФАКТЫ ПО ДАННЫМ ФЕДРЕСУРСА', 'СВЕДЕНИЯ ИЗ РЕЕСТРА ЗАЛОГОВ',
                              'РОСФИНМОНИТОРИНГ(ТЕРРОРИСТИЧЕСКИЕ ОРГАНИЗАЦИИ)', 'САНКЦИИ США, ЕС, КАНАДЫ (ЮЛ)',
                              'ЧЕРНЫЙ СПИСОК ЦБ РФ (ЮЛ)', 'СВЕДЕНИЯ ИЗ СМИ', 'САЙТ В СЕТИ ИНТЕРНЕТ', 'СОЦИАЛЬНЫЕ СЕТИ',
                              'ЛИЗИНГ', 'АНАЛИЗ ДОГОВОРОВ С КОНТРАГЕНТАМИ', 'АРБИТРАЖНЫЕ ДЕЛА', 'ПРОВЕРКА ПАСПОРТА',
                              'Регистрация в качестве индивидуального предпринимателя:', 'АРБИТРАЖНЫЙ СУД ФЛ',
                              'АРБИТРАЖНЫЙ СУД ИП/ЮЛ', 'НАЛОГОВАЯ ЗАДОЛЖЕННОСТЬ 2023',
                              'ЗАБЛОКИРОВАННЫЕ РАСЧЕТНЫЕ СЧЕТА (ФЛ)', 'ФЕДЕРАЛЬНАЯ СЛУЖБА СУДЕБНЫХ ПРИСТАВОВ (ФССП ФЛ)',
                              'РЕЕСТР ЗАЛОГОВ', 'СООБЩЕНИЯ ФЕДРЕСУРС ФЛ', 'НАРКОКОНТРОЛЬ (АРХИВ ФСКН)',
                              'РОСФИНМОНИТОРИНГ: ЭКСТРЕМИСТЫ', 'САНКЦИИ США, ЕС, КАНАДЫ', 'ГАС РФ ПРАВОСУДИЕ',
                              'ОСОБЫЕ РЕЕСТРЫ', 'РЕЕСТР НЕДОБРОСОВЕСТНЫХ ПОСТАВЩИКОВ (РНП)',
                              'ФНС - СЕРВИС ПОЛУЧЕНИЯ ИНН', 'ДОЛЖНИКИ (ФЛ)', 'Учредители ЮЛ', 'Учредители ФЛ',
                              'ПРОВЕРКА НА ДОЛЖНУЮ ОСТМОТРИТЕЛЬНОСТЬ', 'ИСПОЛНИТЕЛЬНЫЕ ПРОИЗВОДСТВА',
                              'АРБИТРАЖНЫЕ ДЕЛА', 'НАИМЕНОВАНИЕ ПЛ', 'ГОД ВЫПУСКА, СОСТОЯНИЕ',
                              'СТОИМОСТЬ ПРЕДМЕТА ЛИЗИНГА', 'ЛИКВИДНОСТЬ', 'ПРАВОУСТАНАВЛИВАЮЩИЕ ДОКУМЕНТЫ',
                              'ПРОВЕРКА ПО ДАННЫМ САЙТА ГИБДД/МИНСЕЛЬХОЗ',
                              'ПРОВЕРКА ПО ДАННЫМ РЕЕСТРА ЗАЛОГОВ ФНП/ФЕДРЕСУРС', 'ПРОВЕРКА ДЕЙСТВИТЕЛЬНОСТИ ЭПТС',
                              '2.1. ДИРЕКТОР/ГЕН. ДИРЕКТОР', '2.2. УЧРЕДИТЕЛИ:', '2.3 ПОРУЧИТЕЛИ:']:
                cell.font = font_bold
            if cell.value in ['Информация о регистрирующем органе:', 'Отсутствуют в реестре данные о контрагенте:',
            ...
            if cell.value in ['Является руководителем:', 'Является учредителем:', 'Являлся руководителем:',
                              'Являлся учредителем:']:
                cell.alignment = alignment_center_bold
                cell.font = font_bold
                cell.fill = color_delta
            ...
            if isinstance(cell.value, str):
                if ('УЧРЕДИТЕЛЬ 1' in cell.value) or ('УЧРЕДИТЕЛЬ 2' in cell.value) or (
                        'УЧРЕДИТЕЛЬ 3' in cell.value) or ('УЧРЕДИТЕЛЬ 4' in cell.value) or (
                        'УЧРЕДИТЕЛЬ 5' in cell.value):
                    cell.font = font_bold
                    cell.fill = color_delta
                    adjacent_cell = sheet.cell(row=cell.row, column=cell.column + 1)
                    adjacent_cell.fill = color_delta
            if isinstance(cell.value, str):
                if 'ЧЕК-ЛИСТ' in cell.value:
                    cell.font = font_bold
            if cell.value in ['1. Сведения из ЕГРЮЛ', '2. Период деятельности', '3. Арбитражные дела',
                              '4. Сведения о банкротстве', '5. Исполнительные производства', '6. Финансовая отчетность',
                              '7. Адреса массовой регистрации', '8. «Массовость» руководителей и участников',
                              '9. Налоговая задолженность', '10. Заблокированные счета',
                              '11. Сведения о причастности к экстремистской деятельности по данным Росфинмониторинг',
                              '12. Черный список ЦБ РФ', '13. Государственные контракты',
                              '14. Реестр недобросовестных поставщиков', '15. Дилерство',
                              '16. Кредитная история в ООО «ЛКМБ-РТ»']:
                cell.font = font_bold
                cell.fill = color_delta
                cell.alignment = alignment_center_bold
                sheet.merge_cells(start_row=cell.row, start_column=cell.column, end_row=cell.row,
                                  end_column=cell.column + 1)


def first_list(wb, name, font_title_name, alignment_wrap, font_a1, font_a2, alignment_center_bold, font_bold, color_a2,
               color_delta, alignment_top_left_bold, alignment_right, font_delta):
    ...
    for cell in sheet['A']:
        if cell.value == '2. Анализ деятельности':
            ...
            for num in range(3, 10):
                sheet.cell(row=cell.row + num, column=cell.column).alignment = alignment_center_bold
    for row in sheet.iter_rows():
        for cell in row:
            if cell.value == 'ФИНАНСОВЫЕ ПОКАЗАТЕЛИ':
                ...
                cell.fill = color_delta
                cell.alignment = alignment_center_bold
                if next_cell.value == 'Отсутствуют финансовые показатели' or next_cell.value == '-':
                    sheet.merge_cells(start_row=cell.row, start_column=cell.column, end_row=cell.row,
                                      end_column=cell.column + 1)
                else:
                    sheet.merge_cells(start_row=cell.row, start_column=cell.column, end_row=cell.row,
                    ...
                    for num in range(1, 4):
                        sheet.cell(row=cell.row + 1, column=cell.column + num).alignment = alignment_right
                        sheet.cell(row=cell.row + 1, column=cell.column + num).font = font_bold

                    for num in range(2, 8):
                        sheet.cell(row=cell.row + num, column=cell.column).alignment = alignment_right
                        sheet.cell(row=cell.row + num, column=cell.column + 1).alignment = alignment_right
                        sheet.cell(row=cell.row + num, column=cell.column + 2).alignment = alignment_right
                        sheet.cell(row=cell.row + num, column=cell.column + 3).alignment = alignment_right
    wb.save(name)
...
def third_list(wb, name, font_title_name, alignment_wrap, font_a2, alignment_center_bold, font_bold, color_a2,
               color_delta, alignment_top_left_bold, alignment_right, font_delta):
    sheet = wb['Продавец']
    overall_design(sheet, font_title_name, alignment_wrap, alignment_center_bold, font_bold, color_delta,
                   alignment_top_left_bold, alignment_right, font_delta)
...
def fourth_list(wb, name, font_title_name, alignment_wrap, font_a2, alignment_center_bold, font_bold, color_a2,
                color_delta, alignment_top_left_bold, alignment_right, font_delta):
