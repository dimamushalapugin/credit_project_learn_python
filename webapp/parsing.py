...
def get_customer_name(request_form):
    ...
    if 'СвЮЛ' in info_about_customer:
        try:
            info_about_customer = info_about_customer['СвЮЛ']['СвНаимЮЛ']['СвНаимЮЛСокр']['@attributes']['НаимСокр']
        except KeyError:
        ...
        if info_about_customer:
            ...
            first_name = fio_rus.get('Имя', '-')
            last_name = fio_rus.get('Фамилия', '-')
            patronymic = fio_rus.get('Отчество', '-')
